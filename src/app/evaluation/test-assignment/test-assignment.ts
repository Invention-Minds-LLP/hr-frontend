...
export class TestAssignment {
  ...
  availFromDate: Date | null = null; // "YYYY-MM-DD"
  availToDate: Date | null = null;
  ...
  onTestChange(testId: number) {
    ...
    if (t?.activeFrom) this.availFromDate = new Date(t.activeFrom);
    if (t?.activeTo) this.availToDate = new Date(t.activeTo);
  }
